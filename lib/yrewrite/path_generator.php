...
class rex_yrewrite_path_generator
{
     ...
     * @param int $clangId The language ID
     * @return string The unique URL
     */
    private function makeUrlUnique(string $url, string $domainName, int $articleId, int $clangId): string
    {
        // Check if URL already exists for a different article
        if (!isset($this->paths[$domainName])) {
            return $url;
        }

        $urlExists = false;
        foreach ($this->paths[$domainName] as $existingArticleId => $clangs) {
            foreach ($clangs as $existingClangId => $existingUrl) {
                // Skip if it's the same article (update case)
                if ($existingArticleId === $articleId && $existingClangId === $clangId) {
                    continue;
                }
    ...
    private function setPath(rex_article $article, rex_yrewrite_domain $domain, string $path)
    {
        ...
        // Make URL unique for auto-generated URLs to avoid duplicates
        if ($isAutoGenerated) {
            $url = $this->makeUrlUnique($url, $domainName, $articleId, $clangId);
        }
